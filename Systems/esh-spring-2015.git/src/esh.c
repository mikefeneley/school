...
main(int ac, char *av[])
{
    ...
    shell_pgid = getpid();
    if(shell_pgid < 0) {
	perror("Get chell pgid error");
    }
    if(setpgid(0, 0)) {
	perror("Set shell pgid error");
    }
  
	

    /* Set up jobs list */
    p_jobs_list = (&jobs_list);
    list_init(p_jobs_list);	
    job_id = 1;

    /* Set up intial terminal state and signal handlers */
    p_state_manager = esh_sys_tty_init();
    give_terminal_to(shell_pgid, p_state_manager);
    sa_sigaction_t p_sig_handler = &sigchld_handler;
    esh_signal_sethandler(SIGCHLD, p_sig_handler);
	...
	/* ENTRY INTO OUR CODE */
	int i = 0;
	int num_jobs = get_num_jobs(cline);

	struct esh_pipeline * job;	
	while(i < num_jobs) {

		job = get_next_job(cline);
		job->pgrp = -1;
		launch_job(job);
		i++;
	}
